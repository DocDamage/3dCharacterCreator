...
| M20 | Color picker | Implemented as the `color_picker` modal with RGB controls and apply/cancel semantics; Penpot visual parity remains simplified | Matrix row 33 is now functionally covered | Continue visual refinement if exact Penpot parity is required |
